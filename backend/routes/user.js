...
// Login
 router.post("/signin", async (req,res) => {
     try{
         ...
         if (!email || !password)
            return res.status(400)({msg: "Not all fields have been entered."});
        
        const user = await User.findOne({email: email});
        
        if (!user)
            return res.status(404)({msg: "No such user has been registered."});
...
router.post("/update", auth, async (req, res) => {
    ...
    User.updateOne({_id: req.user}, updates)
});
